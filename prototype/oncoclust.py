...
def parse_options():
	...
	parser.add_option("-s", "--syn", dest = "syn_mut_fp", type = "string",
                 			action = "store",
					help = "The path to the Synonimous mutations file to construct the background model", metavar = "PATH")
	...
	parser.add_option("-p", "--pos", dest = "pos_pos", type = "int", default = "-1",
                  			action = "store",
                  			help = "Position of the mut files in which the AA position is stated ('-1' by default)", metavar = "INT")

	parser.add_option("-m", "--muts", dest = "min_gene_mutations", type = "int", default = "5",
                  			action = "store",
                  			help = "Minimum number of mutations of a gene to be included in the analysis ('5' by default)", metavar = "INT")
	...
	if not options.syn_mut_fp or not options.non_syn_mut_fp or not os.path.exists(options.syn_mut_fp) or not os.path.exists(options.non_syn_mut_fp):
...
def create_output_file(options, non_syn_accum_mut_pos_dict, non_syn_cluster_coordinates_dict, \
			syn_gene_cluster_scores_dict, non_syn_gene_cluster_scores_dict, cds_dict,  \
...
def get_gene_clusters(gene, gene_len, gene_muts, gene_accum_mut_pos_dict):
	...
	# (note that 'binomial_sig_theshold_per_position' is a global variable)
	minimum_mut_per_position_threshold = get_binomial_minimum_mut_per_position_threshold(gene_len, gene_muts, binomial_sig_theshold_per_position)
	...
	# (applying the constraint of binomial_sig_theshold_per_position, which is a global variable)
	gene_cluster_dict = group_meaningful_positions(gene, significant_gene_positions_l)
...
'''
def cluster_mutations(accum_mut_pos_dict, min_gene_mutations, cds_dict):
...
Given several positions, I group them by applying a constraint that the maximal
 distance between positions in the cluster is = intra_cluster_max_distance
'''
...
def run_oncoclust(options):
	...
	non_syn_cluster_coordinates_dict = cluster_mutations(non_syn_accum_mut_pos_dict, min_gene_mutations, cds_dict)
	print '\tCoding_silent mutations..'
	syn_cluster_coordinates_dict = cluster_mutations(syn_accum_mut_pos_dict, min_gene_mutations, cds_dict)
...
if __name__ == "__main__":
	...
	binomial_sig_theshold_per_position = 0.01
